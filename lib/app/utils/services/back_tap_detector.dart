...
class BackTapDetector {
  ...
  BackTapDetector({
    ...
    if (kDebugMode) {
      print(
        "BTD INIT: Thresh(PI=$tapPIThreshold, Mag=$minPeakMagnitude, GyroImpact=$gyroRejectionThreshold)",
      );
      print(
        "BTD INIT: Quiet(MoveLimit=$quietMovementMax, Window=${quietWindowMs}ms)",
      );
    }
  }
  ...
  void start() {
    ...
    _userAccelSub = userAccelerometerEventStream().listen((event) {
      ...
      if (_lastSuccessTime != null &&
          now.difference(_lastSuccessTime!).inMilliseconds < cooldownMs) {
        return;
      }
      ...
      double pi = 0;
      if (_prevAccel != null) {
        dx = event.x - _prevAccel!.x;
        dy = event.y - _prevAccel!.y;
        dz = event.z - _prevAccel!.z;
        pi = dx.abs() + dy.abs() + dz.abs();
      }
      ...
      _processInternal(now, pi, rawMag, dx, dy, dz);
  ...
  void _processInternal(
    ...
    final elapsed = _stateStartTime == null
        ? 0
        : now.difference(_stateStartTime!).inMilliseconds;
    ...
    switch (_state) {
      case _BTDState.idle:
        ...
        // --- Z-Axis Dominance & Polarity Checks ---
        // A back tap generates a strong positive Z acceleration.
        // It should also be the dominant movement axis.
        bool isZDominant =
            dz.abs() > (dx.abs() * 1.3) && dz.abs() > (dy.abs() * 1.3);
        bool isPositiveZ = dz > (tapPIThreshold * 0.7);

        if (pi > 1.0 && rawMag > 0.8) {
          if (kDebugMode && rawMag > 1.3) {
            print(
              "BTD POTENTIAL: PI=${pi.toStringAsFixed(2)} Mag=${rawMag.toStringAsFixed(2)} dz=${dz.toStringAsFixed(2)} Z-Dom=$isZDominant",
            );
          }
        }

        if (pi > tapPIThreshold && rawMag > minPeakMagnitude) {
          if (!isZDominant || !isPositiveZ) {
            if (kDebugMode && rawMag > 1.3)
              print(
                "BTD REJECT: Tap was not a Back Tap (Front/Shake). dz=${dz.toStringAsFixed(2)}",
              );
            return;
          }
          ...
          if (stillInfo.isStill) {
            if (avgGyro < gyroRejectionThreshold) {
              ...
              if (kDebugMode)
                print(
                  "BTD >>> TAP 1 STARTED (GyroAvg=${avgGyro.toStringAsFixed(2)}) Success still: ${stillInfo.report}",
                );
            } else {
              if (kDebugMode && rawMag > 1.5)
                print(
                  "BTD REJECT: Gyro too high during Tap 1 ($avgGyro) < $gyroRejectionThreshold",
                );
            }
          } else {
            if (kDebugMode && rawMag > 1.5)
              print(
                "BTD REJECT: Phone not still before Tap 1 (${stillInfo.report})",
              );
          }
        }
        break;
      ...
      case _BTDState.potentialTap1:
        ...
        if (isDecayed) {
          ...
          if (kDebugMode)
            print(
              "BTD >>> TAP 1 DECAY COMPLETE after ${elapsed}ms (Mag=${rawMag.toStringAsFixed(2)}) - Entering Quiet Window.",
            );
        } else if (elapsed > maxSpikeDurationMs) {
          if (kDebugMode)
            print(
              "BTD REJECT: Spike exceeded $maxSpikeDurationMs ms without decay. Possible shake/sustained movement. (Mag=${rawMag.toStringAsFixed(2)} | Gyro=${avgGyro.toStringAsFixed(2)})",
            );
          _resetMachine();
        } else if (avgGyro > 0.80) {
          // Safety: if gyro is very high during decay, reset.
          if (kDebugMode)
            print("BTD REJECT: Excessive rotation during decay ($avgGyro)");
          _resetMachine();
        }
        break;
      ...
      case _BTDState.quietWindow:
        ...
        if (avgGyro > quietMovementMax) {
          if (kDebugMode)
            print(
              "BTD REJECT: Movement during quiet window ($avgGyro > $quietMovementMax)",
            );
          _resetMachine();
          return;
        }
        ...
        if (elapsed >= quietWindowMs) {
          ...
          if (kDebugMode) {
            ...
            print(
              "BTD QUIET PASSED: movement=${avgGyro.toStringAsFixed(3)} [Limit $quietMovementMax] → Waiting for TAP 2 (Window: ${maxDoubleTapDelayMs}ms, Remaining: ${remaining}ms)",
            );
          }
        }
        break;
      ...
      case _BTDState.waitingForTap2:
        ...
        if (pi > (tapPIThreshold * 0.65) &&
            rawMag > (minPeakMagnitude * 0.65)) {
          // We enforce polarity on the 2nd tap but slightly relaxed dominance
          bool isPositiveZ = dz > (tapPIThreshold * 0.4);
          bool isZDominant = dz.abs() > dx.abs() && dz.abs() > dy.abs();

          if (!isPositiveZ || !isZDominant) {
            if (kDebugMode)
              print(
                "BTD REJECT: Tap 2 lacked Z-polarity (dz=${dz.toStringAsFixed(2)})",
              );
            _resetMachine();
            return;
          }
          ...
          if (avgGyro < gyroRejectionThreshold) {
            ...
            if (kDebugMode)
              print(
                "BTD >>> TAP 2 SUCCESS - DOUBLE BACK TAP TRIGGERED [Delay: ${elapsed}ms]",
              );
            onDoubleTap();
          } else {
            if (kDebugMode)
              print(
                "BTD REJECT: Tap 2 gyro too high ($avgGyro > $gyroRejectionThreshold)",
              );
            _resetMachine();
          }
        } else if (elapsed > maxDoubleTapDelayMs) {
          if (kDebugMode)
            print("BTD TIMEOUT: Tap 2 window timed out after ${elapsed}ms.");
          _resetMachine();
        }
        break;
    }
  ...
  double _getRecentGyroAverage(int sampleCount) {
    ...
    final int count = sampleCount > _gyroHistory.length
        ? _gyroHistory.length
        : sampleCount;
    double sum = 0;
  ...
  _StillnessReport _getStillnessReport() {
    ...
    if (filtered.length < 5)
      return _StillnessReport(true, "No recent stability history");
    ...
    bool isOk =
        (variance < stillnessAccelVarianceMax) && (gMean < stillnessGyroMax);
    String msg =
        "Var=${variance.toStringAsFixed(3)} [Max $stillnessAccelVarianceMax] | Gyro=${gMean.toStringAsFixed(3)} [Max $stillnessGyroMax] (Used ${filtered.length} samples)";
